...
public final class TankItemRenderer extends BlockEntityWithoutLevelRenderer
{
    ...
    public void renderByItem(ItemStack stack, ItemDisplayContext ctx, PoseStack poseStack, MultiBufferSource buffer, int light, int overlay)
    {
        ...
        FluidRenderHandler handler = FluidRenderHandlerRegistry.INSTANCE.get(content.fluid());
        if (handler == null) return;

        FluidState fluidState = content.fluid().defaultFluidState();
        TextureAtlasSprite[] sprites = handler.getFluidSprites(null, null, fluidState);
        int tint = handler.getFluidColor(null, null, fluidState);
        RenderType renderType = ItemBlockRenderTypes.getRenderLayer(fluidState);

        FramedTankRenderer.renderContents(poseStack, buffer, renderType, light, content.amount(), sprites[0], sprites[1], tint);
    }
}

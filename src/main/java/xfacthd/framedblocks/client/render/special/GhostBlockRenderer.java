...
public final class GhostBlockRenderer
{
    ...
    /**
     * Registered via {@link net.fabricmc.fabric.api.client.rendering.v1.WorldRenderEvents#AFTER_PARTICLES}.
     */
    public static void onRenderAfterParticles(net.fabricmc.fabric.api.client.rendering.v1.WorldRenderContext ctx)
    {
        if (!ClientConfig.VIEW.showGhostBlocks()) return;

        ProfilerFiller profiler = mc().getProfiler();
        profiler.push(PROFILER_KEY);
        try
        {
            tryDrawGhostBlock(ctx.matrixStack(), profiler);
        }
        catch (Throwable t)
        {
    ...
    )
    {
        RenderType bufferType = ClientConfig.VIEW.useAltGhostRenderer() ?
                ...
                net.minecraft.client.renderer.RenderType.translucent().get();

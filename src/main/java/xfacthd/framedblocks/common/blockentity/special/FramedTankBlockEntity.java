...
public class FramedTankBlockEntity extends FramedBlockEntity
{
    ...
    {
        // TODO (Step 6 - Capabilities): Replace with Fabric fluid interaction helper
        // FluidUtil.interactWithFluidHandler is NeoForge-specific
        // Fabric equivalent: FluidStorage.getFluidContained() + bucket interaction
        return ItemInteractionResult.PASS_TO_DEFAULT_BLOCK_INTERACTION;
    }

    public FluidContent getContents()
    {
        return fluidHandler.getFluidContent();
    }

    public xfacthd.framedblocks.common.capability.TankFluidHandler getFluidHandler()
    {
        return fluidHandler;
    }
    ...
    {
        FluidContent contents = input.getOrDefault(FBContent.DC_TYPE_TANK_CONTENTS.value(), FluidContent.EMPTY);
        if (!contents.isEmpty())
        {
            fluidHandler.setFluidContent(contents);
        }
    }
    ...
    {
        FluidContent contents = fluidHandler.getFluidContent();
        if (!contents.isEmpty())
        {
            builder.set(FBContent.DC_TYPE_TANK_CONTENTS.value(), contents);
        }
    }

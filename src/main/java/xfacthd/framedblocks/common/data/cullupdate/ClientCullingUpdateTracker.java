...
{
    public static void handleCullingUpdates(ClientboundCullingUpdatePayload payload, ClientPlayNetworking.Context ctx)
    {
        CullingUpdateChunk chunk = new CullingUpdateChunk(new ChunkPos(payload.chunk()), payload.positions());
        ctx.client().execute(() -> handleCullingUpdates(chunk));
    }

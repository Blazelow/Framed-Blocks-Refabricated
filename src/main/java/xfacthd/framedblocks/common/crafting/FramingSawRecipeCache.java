...
public final class FramingSawRecipeCache
{
    ...
    /**
     * Register the recipe-cache update hook.
     * Called from {@link xfacthd.framedblocks.FramedBlocks#onInitialize()}.
     *
     * On Fabric, recipe reloads are signalled via {@code ServerLifecycleEvents.END_DATA_PACK_RELOAD}
     * and on initial server start via {@code ServerLifecycleEvents.SERVER_STARTED}.
     */
    public static void registerReloadListener()
    {
        net.fabricmc.fabric.api.event.lifecycle.v1.ServerLifecycleEvents.SERVER_STARTED.register(server ->
                SERVER_INSTANCE.update(server.getRecipeManager())
        );
